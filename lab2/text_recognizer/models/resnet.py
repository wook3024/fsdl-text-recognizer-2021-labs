...
class ResNet(nn.Module):
    def __init__(
        ...
        *,
        ...
        model_config = model_config_set.get(model_class)

        block = model_config.get("block")
        layers = model_config.get("layers")
        groups = model_config.get("groups", 1)
        width_per_group = model_config.get("width_per_group", 64)
        zero_init_residual = args.get("zero_init_residual", False)
        replace_stride_with_dilation = args.get("replace_stride_with_dilation", None)
    ...
    def add_to_argparse(parser):
        parser.add_argument("--pretrained", type=int, default=PRE_TRAINED)
        parser.add_argument("--groups", type=int, default=GROUPS)
        parser.add_argument("--width_per_group", type=int, default=WIDTH_PER_GROUP)
        return parser
